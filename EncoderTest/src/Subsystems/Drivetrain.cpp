...
*/
void Drivetrain::userDrivebackward(std::shared_ptr<Joystick>xbox){
	fl->SetControlMode(CANTalon::kPercentVbus);
	fr->SetControlMode(CANTalon::kPercentVbus);
...
void Drivetrain::userDriveslow(std::shared_ptr<Joystick>xbox){
	fl->SetControlMode(CANTalon::kPercentVbus);
	fr->SetControlMode(CANTalon::kPercentVbus);
...
bool Drivetrain::isMove(){
	if ((fl->GetPosition() < (l_pos + .1) && fl->GetPosition() > (l_pos - .1))
				&& fr->GetPosition() < (r_pos + .1)
				&& fr->GetPosition() > (r_pos - .1)) {
			return true;
		} else {
			return false;
